...
public record BookingResponse(Long id, Long clientId, Integer peopleAmount, List<RoomResponse> rooms,
                              ...
                              BigDecimal total, boolean active) {
    public BookingResponse(Booking booking) {
        this(booking.getId(), booking.getClientId(), booking.getPeopleAmount(), booking.getRooms().stream().map(RoomResponse::new).toList(), booking.getProductItems().stream().map(ProductItemResponse::new).toList(), booking.getCheckIn(), booking.getCheckOut(), booking.getTotal(), booking.isActive());
    }
}